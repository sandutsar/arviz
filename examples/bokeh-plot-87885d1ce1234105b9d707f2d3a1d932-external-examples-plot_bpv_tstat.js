(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("4f04ed44-c6d5-49ff-8027-16bd1e2985c0");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '4f04ed44-c6d5-49ff-8027-16bd1e2985c0' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.3.3.min.js": "dM3QQsP+wXdHg42wTqW85BjZQdLNNIXqlPw/BgKoExPmTG7ZLML4EGqLMfqHT6ON", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.3.min.js": "8x57I4YuIfu8XyZfFo0XVr2WAT8EK4rh/uDe3wF7YuW2FNUSNEpJbsPaB1nJ2fz2", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.3.min.js": "3QTqdz9LyAm2i0sG5XTePsHec3UHWwVsrOL68SYRoAXsafvfAyqtQ+h440+qIBhS"};
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.3.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.3.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"8c487dda-2a08-4d77-94cd-c5f1d6f7f4ad":{"defs":[],"roots":{"references":[{"attributes":{},"id":"15927","type":"AllLabels"},{"attributes":{"data_source":{"id":"15918"},"glyph":{"id":"15919"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"15920"},"view":{"id":"15922"}},"id":"15921","type":"GlyphRenderer"},{"attributes":{},"id":"15936","type":"Selection"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"15941","type":"Circle"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"15915","type":"Line"},{"attributes":{"data":{},"selected":{"id":"15936"},"selection_policy":{"id":"15935"}},"id":"15918","type":"ColumnDataSource"},{"attributes":{},"id":"15930","type":"AllLabels"},{"attributes":{},"id":"15900","type":"SaveTool"},{"attributes":{"toolbar":{"id":"15963"},"toolbar_location":"above"},"id":"15964","type":"ToolbarBox"},{"attributes":{},"id":"15928","type":"BasicTickFormatter"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"15902","type":"BoxAnnotation"},{"attributes":{},"id":"15895","type":"PanTool"},{"attributes":{},"id":"15925","type":"BasicTickFormatter"},{"attributes":{},"id":"15897","type":"WheelZoomTool"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"15914","type":"Line"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"15919","type":"Line"},{"attributes":{"source":{"id":"15939"}},"id":"15943","type":"CDSView"},{"attributes":{},"id":"15934","type":"Selection"},{"attributes":{},"id":"15960","type":"Selection"},{"attributes":{"axis":{"id":"15890"},"dimension":1,"ticker":null},"id":"15893","type":"Grid"},{"attributes":{"overlay":{"id":"15902"}},"id":"15896","type":"BoxZoomTool"},{"attributes":{"callback":null},"id":"15901","type":"HoverTool"},{"attributes":{},"id":"15880","type":"DataRange1d"},{"attributes":{"overlay":{"id":"15903"}},"id":"15898","type":"LassoSelectTool"},{"attributes":{"active_multi":null,"tools":[{"id":"15894"},{"id":"15895"},{"id":"15896"},{"id":"15897"},{"id":"15898"},{"id":"15899"},{"id":"15900"},{"id":"15901"}]},"id":"15904","type":"Toolbar"},{"attributes":{},"id":"15882","type":"LinearScale"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"15920","type":"Line"},{"attributes":{"below":[{"id":"15886"}],"center":[{"id":"15889"},{"id":"15893"},{"id":"15937"}],"height":500,"left":[{"id":"15890"}],"output_backend":"webgl","renderers":[{"id":"15916"},{"id":"15921"},{"id":"15942"}],"title":{"id":"15944"},"toolbar":{"id":"15904"},"toolbar_location":null,"width":500,"x_range":{"id":"15878"},"x_scale":{"id":"15882"},"y_range":{"id":"15880"},"y_scale":{"id":"15884"}},"id":"15877","subtype":"Figure","type":"Plot"},{"attributes":{"data_source":{"id":"15939"},"glyph":{"id":"15940"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"15941"},"view":{"id":"15943"}},"id":"15942","type":"GlyphRenderer"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"15940","type":"Circle"},{"attributes":{"children":[{"id":"15964"},{"id":"15962"}]},"id":"15965","type":"Column"},{"attributes":{"data_source":{"id":"15913"},"glyph":{"id":"15914"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"15915"},"view":{"id":"15917"}},"id":"15916","type":"GlyphRenderer"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"15921"}]},"id":"15938","type":"LegendItem"},{"attributes":{},"id":"15899","type":"UndoTool"},{"attributes":{},"id":"15933","type":"UnionRenderers"},{"attributes":{},"id":"15894","type":"ResetTool"},{"attributes":{"data":{},"selected":{"id":"15960"},"selection_policy":{"id":"15959"}},"id":"15939","type":"ColumnDataSource"},{"attributes":{"items":[{"id":"15938"}]},"id":"15937","type":"Legend"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"15934"},"selection_policy":{"id":"15933"}},"id":"15913","type":"ColumnDataSource"},{"attributes":{},"id":"15891","type":"BasicTicker"},{"attributes":{"source":{"id":"15913"}},"id":"15917","type":"CDSView"},{"attributes":{},"id":"15878","type":"DataRange1d"},{"attributes":{},"id":"15884","type":"LinearScale"},{"attributes":{},"id":"15887","type":"BasicTicker"},{"attributes":{},"id":"15935","type":"UnionRenderers"},{"attributes":{"formatter":{"id":"15925"},"major_label_policy":{"id":"15927"},"ticker":{"id":"15887"}},"id":"15886","type":"LinearAxis"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"15903","type":"PolyAnnotation"},{"attributes":{"toolbars":[{"id":"15904"}],"tools":[{"id":"15894"},{"id":"15895"},{"id":"15896"},{"id":"15897"},{"id":"15898"},{"id":"15899"},{"id":"15900"},{"id":"15901"}]},"id":"15963","type":"ProxyToolbar"},{"attributes":{"formatter":{"id":"15928"},"major_label_policy":{"id":"15930"},"ticker":{"id":"15891"}},"id":"15890","type":"LinearAxis"},{"attributes":{"children":[[{"id":"15877"},0,0]]},"id":"15962","type":"GridBox"},{"attributes":{"axis":{"id":"15886"},"ticker":null},"id":"15889","type":"Grid"},{"attributes":{"source":{"id":"15918"}},"id":"15922","type":"CDSView"},{"attributes":{"text":"y / y","text_font_size":"15pt"},"id":"15944","type":"Title"},{"attributes":{},"id":"15959","type":"UnionRenderers"}],"root_ids":["15965"]},"title":"Bokeh Application","version":"2.3.3"}}';
                  var render_items = [{"docid":"8c487dda-2a08-4d77-94cd-c5f1d6f7f4ad","root_ids":["15965"],"roots":{"15965":"4f04ed44-c6d5-49ff-8027-16bd1e2985c0"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();